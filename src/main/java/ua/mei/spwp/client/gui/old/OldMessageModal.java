...
public class OldMessageModal extends BaseOwoScreen<FlowLayout> {
    public MutableText title;
    public MutableText message;

    public OldMessageModal(MutableText title, MutableText message) {
        this.title = title;
        this.message = message;
    }

    public static void openMessage(MutableText title, MutableText message) {
        MinecraftClient.getInstance().setScreen(new OldMessageModal(title, message));
    }
    ...
    protected void build(FlowLayout rootComponent) {
        rootComponent.child(
                        Containers.verticalFlow(Sizing.fill(60), Sizing.content())
                                ...
                                .child(Components.button(Text.translatable("gui.spwp.button.ok"), button -> {
